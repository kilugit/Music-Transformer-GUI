...
# Default MIDI timing parameters for tick↔ms conversion
DEFAULT_TEMPO = 500000       # default tempo in µs/beat (120 BPM)
DEFAULT_TPB = 480            # default ticks per beat
...
def _build_vocab_index(vocab_list):
    """Build O(1) lookup dict from vocab list."""
    return {event: i for i, event in enumerate(vocab_list)}


_vocab_index = _build_vocab_index(vocab)
...
def events_to_indices(event_list, _vocab=None):
    ...
    return [_vocab_index[event] for event in event_list]
