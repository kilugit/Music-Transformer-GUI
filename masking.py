...
def _cached_look_ahead_mask(seq_len, device):
    mask = torch.triu(torch.ones(seq_len, seq_len, device=torch.device(device)), diagonal=1).float()
    return mask


def create_look_ahead_mask(seq_len, device='cpu'):
    return _cached_look_ahead_mask(seq_len, device)


def create_mask(inp, n=4):
    padding_mask = create_padding_mask(inp, n=n)
    look_ahead_mask = _cached_look_ahead_mask(inp.shape[-1], str(inp.device))
    combined_mask = (padding_mask + look_ahead_mask).clamp(max=1)
...
def create_cache_mask(inp, past_length, n=4):
    ...
    look_ahead = _cached_look_ahead_mask(seq_len_q, str(inp.device))
