...
class MultiHeadAttention(nn.Module):
    ...
    def get_required_embeddings(self, seq_len, max_len=None):
        ...
        if n_extra > 0:
            ...
            return torch.cat([first.expand(n_extra, -1),
                              self.E(torch.arange(start, max_len, device=E_dev))], dim=0)
        else:
            return self.E(torch.arange(start, max_len, device=E_dev))
    ...
    def forward(self, q, k, v, mask=None, past_k=None, past_v=None):
        ...
        # compute MHA
        rel_scaled_attention = rel_scaled_dot_prod_attention(q, k_h, v_h, e, mask=mask)
        ...
        attn_out = self.wo(rel_scaled_attention.reshape(*sh[:-2], self.d_model))
